...
4. **Поиск URI MAC-правил** — из LDAP выбираются все `aldURIMACRule`, у которых `x-ald-uri-service-ref` содержит DN HTTP-службы целевого хоста.
...
При старте сервис загружает все правила из LDAP и разрешает ссылки на службы.
